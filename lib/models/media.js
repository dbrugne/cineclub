...
function getApiData() {
  ...
  let videos;
  if (info.videos && info.videos.results && info.videos.results.length) {
    videos = info.videos.results.filter(v => v.site === 'YouTube').map(({ key, name }) => ({
      key,
      name,
    }));
  }
  ...
  let images;
  if (info.images && info.images.backdrops && info.images.backdrops.length) {
    images = info.images.backdrops.map(({ file_path, height, width }) => ({
      file_path,
      height,
      width,
    }));
  ...
  const credits = {};
  if (info.credits) {
    if (info.credits.crew && info.credits.crew.length) {
      const direction = info.credits.crew
    ...
    if (info.credits.cast) {
      const cast = info.credits.cast
        .filter(p => p.order <= 10)
        .map(({ order, name, character, profile_path }) => ({
          order,
          character,
          name,
          profile_path,
