    ...
    sequence = sequence
      ...
      .then(() => {
        if (doc.info && doc.info.id && doc.info && doc.info.category) {
          debug(`DECORATE :: using existing ${doc.info.id} and ${doc.info.category}`);
      ...
      .then(({ type, id }) => {
        if (!type || !id) {
          debug(`DECORATE :: failed to find type/id for ${doc.path}`);
          return undefined;
        }
      ...
      .then(info => {
        if (!info
          || !info.id
          || info.category !== 'tv'
          || !doc.torrent || !doc.torrent.season || !doc.torrent.episode) {
          return info;
        }

        return episode(opts.api, info.id, doc.torrent.season, doc.torrent.episode)
          .then(episodeInfo => Object.assign({ episode: episodeInfo }, info));
      })
      .then(info => {
