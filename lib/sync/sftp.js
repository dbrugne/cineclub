...
const listFiles = (sftp, filepath, shouldBeIgnored) => new Promise((done, fail) => {
  if (shouldBeIgnored(filepath)) {
    debug('SFTP :: ignoring', filepath);
    return done([]);
  }
  ...
  return statAsync(filepath)
    .then(stats => {
      if (!stats.isDirectory()) {
        debug('SFTP :: file found', filepath);
      ...
      debug('SFTP :: directory found', filepath);
